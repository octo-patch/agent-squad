...
class MiniMaxClassifierOptions:
    def __init__(self,
                 api_key: str,
...
class MiniMaxClassifier(Classifier):
    def __init__(self, options: MiniMaxClassifierOptions):
        super().__init__()
        ...
        self.model_id = options.model_id or MINIMAX_MODEL_ID_M2_7

        default_max_tokens = 1000
        # Clamp temperature for MiniMax API (must be > 0 when set)
        temperature = options.inference_config.get('temperature', 0.1)
        if temperature <= 0:
            temperature = 0.01
